...
function getAdaptiveTimeout(endpoint: string): number {
...
(window as any).dataLoader = {
  async load(endpoint: string, options: LoadOptions = {}): Promise<any> {
    const {
      fallback = null,
  ...
  async _fetchDirect(endpoint: string, { fallback, timeout, retries, deepCopy = true }: { fallback: any; timeout: number; retries: number; deepCopy?: boolean }): Promise<any> {
  ...
  async loadMany(endpoints: string[], options: LoadOptions = {}): Promise<LoadManyResult[]> {
