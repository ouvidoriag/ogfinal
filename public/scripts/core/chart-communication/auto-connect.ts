...
(function() {
  ...
  if (!win.eventBus) {
    if ((window as any).Logger) {
      (window as any).Logger.error('eventBus global não encontrado. Verifique se event-bus.js está carregado antes de auto-connect.js');
    }
   ...
   */
  function createPageFilterListener(
    pageId: string, 
    reloadFunction: PageLoaderFunction, 
   ...
   */
  function autoConnectAllPages(): void {
    if (!window.chartCommunication) {
      return;
    }
    
    // Mapeamento de páginas para suas funções de carregamento
    const pageLoaders: Record<string, PageLoaderFunction | undefined> = {
