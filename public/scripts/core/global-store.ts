...
type DataStoreListener = (data: any, key: string) => void;
...
const dataStore = {
  ...
  dashboardDataTimestamp: null as number | null,
  dataCache: new Map<string, any>(),
  dataTimestamps: new Map<string, number>(),
  listeners: new Map<string, Set<DataStoreListener>>(),
  defaultTTL: 5000,
...
function createDeepCopy(data: any): any {
  if (data === null || data === undefined) return data;
  
  // Proteção básica: verificar se contém objetos Chart.js antes de tentar serializar
  function hasChartObjects(obj: any, depth: number = 0): boolean {
...
function getEffectiveTTL(key: string): number {
  for (const [pattern, ttl] of Object.entries(dataStore.ttlConfig)) {
    if (pattern.includes('*')) {
      const regex = new RegExp(pattern.replace(/\*/g, '.*'));
      if (regex.test(key)) return ttl;
...
function getPersistent(key: string, ttl: number | null = null): any {
  try {
    const storageKey = dataStore.persistentPrefix + key;
    const cached = localStorage.getItem(storageKey);
    if (cached) {
      const { data, timestamp, ttl: storedTTL }: PersistentCacheData = JSON.parse(cached);
...
function setPersistent(key: string, data: any, ttl: number | null = null): void {
  try {
    const storageKey = dataStore.persistentPrefix + key;
...
function clearOldPersistent(): void {
  try {
    ...
    keys.forEach(k => {
      if (k.startsWith(dataStore.persistentPrefix)) {
        try {
          const cached: PersistentCacheData | null = JSON.parse(localStorage.getItem(k) || 'null');
...
function get(key: string, ttl: number | null = null, returnCopy: boolean = false): any {
...
function set(key: string, data: any, deepCopy: boolean = false): void {
...
function notifyListeners(key: string, data: any): void {
...
function subscribe(key: string, callback: DataStoreListener): () => void {
...
function clear(key: string | null = null): void {
  if (key === null) {
...
function clearPersistent(key: string | null = null): void {
  try {
    if (key === null) {
      const keys = Object.keys(localStorage);
      keys.forEach(k => {
...
function invalidate(keys?: string[] | string): void {
  if (!keys) {
    clear(null);
    return;
  }
...
function getStats(): DataStoreStats {
  return {
...
if (typeof window !== 'undefined') {
  ...
  window.dataStore = {
    ...
    setDefaultTTL: (ttl: number) => { dataStore.defaultTTL = ttl; },
    getPersistent,
    setPersistent,
    clearPersistent,
    clearOldPersistent
